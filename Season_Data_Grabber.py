...
event_id = event_list_import["Event ID"].tolist()
...
for event in event_id:
    ...
    session.load()
    event_id_inc = event_id[index]
    laps = session.laps
    weather_data = session.laps.get_weather_data()
    laps = laps.reset_index(drop=True)
    weather_data = weather_data.reset_index(drop=True)
    event_list = {'Year': [year[index]], 'Grand_Prix': [grand_prix[index]], 'Event_List': [grand_prix_day[index]], 'Event_ID': [event_id_inc]}
...
com_df.to_excel("C:/Users/johnp/Downloads/season2022.xlsx")
